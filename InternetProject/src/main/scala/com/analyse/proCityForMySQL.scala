...
object proCityForMySQL {
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().config(conf).appName("proCityForMySQL").master("local[1]").getOrCreate()
    ...
    // 编写sql语句。
    val sql = "select provincename,cityname,count(*) as pccount from log group by provincename,cityname"
